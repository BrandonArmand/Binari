...
function positionNode(node, layer, xOrder, windowSize, nodeList) {
    node.layer = layer;
    node.xOrder = xOrder;
    ...
    nodeList.push(node);
    if (node.left != null) {
        positionNode(node.left, layer + 1, xOrder * 2 - 1, windowSize, nodeList);
    }
    if (node.right != null) {
        positionNode(node.right, layer + 1, xOrder * 2, windowSize, nodeList);
    }
}
...
export function setTree(tree, nodeList) {
    nodeList = [];
